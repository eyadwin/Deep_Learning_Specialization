...
#tf.enable_eager_execution()# enabled by default in TensorFlow 2.0. Needed only to TensorFlow 1.x
...
def compute_cost(logits, labels):
    ...
    # YOUR CODE STARTS HERE
    cost = tf.reduce_mean(
        tf.reduce_mean(tf.maximum(logits, 0) - logits * labels + tf.math.log(1 + tf.math.exp(-abs(logits))), axis=-1))
